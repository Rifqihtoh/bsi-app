...
const validateId = (req, res, next) => {
  ...
  // validaso untuk memastikan data baru tidak memiliki ID duplikasi
  const validateDuplikasi = (req, res, next) => {
    const itemId = parseInt(req.body.id);
    const existingItem = bsiTrainee.find(item => item.id === itemId);

    if (existingItem) {
      return res.status(400).json({ message: 'ID sudah ada, mohon buat id baru' });
    }
    next();
  }
...
app.get('/api/items/:id', validateId, (req, res) => {
  ...
  if (item) {
    ...
    res.status(404).json({ message: 'Manusia Hilang' });
  }
});

// POST method untuk menambahkan data baru
app.post('/api/items',validateAdd, validateDuplikasi,(req, res) => {
